...
class Text {
    ...
    getWidth() {
        ...
        for (let i = 0; i < this.lines.length; i++) {
            const lineWidth = this.measureWidth(this.lines[i]);
            if (max < lineWidth) {
                max = lineWidth;
            }
        }
    ...
    applyMultiline() {
        if (this.maxWidth === 0) {
            ...
            const tempLines = this.text.split("\n");
            for (let tempLineI = 0; tempLineI < tempLines.length; tempLineI++) {
                const words = tempLines[tempLineI].split(" ");

                let currentLine = "";
                for (let partI = 0; partI < words.length; partI++) {
    ...
    drawLine(c, line, fullLine, x, y, drawBorder) {
        if (this.letterSpacing === 0 && this.monospaced === -1) {
            if (this.finishedAppearing || this.align === "left") {
                ...
                let appearOffset = this.measureWidth(fullLine) - this.measureWidth(line);
